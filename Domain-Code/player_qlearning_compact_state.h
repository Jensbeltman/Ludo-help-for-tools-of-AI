...
private:
    ...
    void update_pre_round_reward()
    {
        ...
        if (positionDiff > 0 || move_eval[action].collisions == 1) // if we didnt get sent home and reward is increased and collision happened
            reward += 50;
    ...
    void update_qtable(){
        ...
        qTable->operator[](state_prev)[action] += alpha * (reward + gamma * stateMaxQ - qTable->operator[](state_prev)[action] );
    }
